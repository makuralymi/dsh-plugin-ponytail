window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		var WhipSimulation = class {
			...
			*/
			constructor(options = {}) {
				...
				this.damping = options.damping ?? .98;
			...
			*/
			step(dt) {
				const frameScale = Math.max(dt, .001) * 60;
				if (this.phase === "charge") {
					this.chargeLevel = Math.min(1, this.chargeLevel + .03 * frameScale);
					this.targetAngle = this.idleAngle + this.chargeLevel * this.chargeLean;
					this.currentAngle += (this.targetAngle - this.currentAngle) * .15;
				} else if (this.phase === "strike_forward") {
					this.targetAngle = this.idleAngle - this.strikeSwing - this.strikeForce * this.strikeForceSwing;
					this.currentAngle += (this.targetAngle - this.currentAngle) * .6;
					...
					this.targetAngle = this.idleAngle;
					this.currentAngle += (this.targetAngle - this.currentAngle) * .5;
					...
					this.targetAngle = this.idleAngle;
					this.currentAngle += (this.targetAngle - this.currentAngle) * .1;
				}
				const rad = this.currentAngle * Math.PI / 180;
				...
				for (let iter = 0; iter < this.iterations; iter += 1) for (let i = 0; i < this.points.length - 1; i += 1) {
					const a = this.at(i);
					const b = this.at(i + 1);
					const dx = b.x - a.x;
					const dy = b.y - a.y;
					...
					if (dist === 0) continue;
					const percent = (this.segmentLength - dist) / dist / 2;
					const ox = dx * percent;
					const oy = dy * percent;
					if (i !== 0) {
						a.x -= ox;
		...
		*/
		function fireCrack(sim, sparks, now, inputActions, lastHurryRef, pickPrompt, shouldInterrupt, cancelTurn) {
			const rope = sim.rope();
		...
		*/
		function WhipOverlay({ inputActions, pickPrompt, shouldInterrupt, cancelTurn, onDisarm }) {
		...
		*/
		...
		const PONYTAIL_INTERRUPT_FIELD = "interrupt";
		...
		*/
		function parsePonytailSettings(value) {
			...
			const raw = value;
			if (raw["interrupt"] !== void 0 && typeof raw["interrupt"] !== "boolean") return void 0;
			...
			return {
				groups,
				interrupt: raw[PONYTAIL_INTERRUPT_FIELD] === true
			};
		}
		//#endregion
		...
		var PonytailSettingsSection_module_css_default = {
			...
			"behaviorSwitch": "R7ek2q_behaviorSwitch",
			"textarea": "R7ek2q_textarea",
			"group": "R7ek2q_group",
			"textInput": "R7ek2q_textInput",
			"select": "R7ek2q_select",
			"behaviorDescription": "R7ek2q_behaviorDescription",
			...
			return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
				...
				children: [
					...
					/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("label", {
						className: PonytailSettingsSection_module_css_default.behaviorSwitch,
						children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("input", {
							type: "checkbox",
							className: PonytailSettingsSection_module_css_default.behaviorCheckbox,
							checked: settings.interrupt,
							onChange: (event) => {
								controller.setInterrupt(event.target.checked);
							}
						}), /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("span", {
							className: PonytailSettingsSection_module_css_default.behaviorCopy,
							children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("strong", {
								className: PonytailSettingsSection_module_css_default.behaviorTitle,
								children: "终止当前任务后发送"
							}), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
								className: PonytailSettingsSection_module_css_default.behaviorDescription,
								children: "默认关闭。开启后，模型正在工作时抽鞭会先终止当前任务，再立即把催促提示词作为新消息发送； 关闭时催促消息进入队列，等待当前任务处理完。"
							})]
						})]
					}),
		...
		var PonytailSettingsController = class {
			...
			*/
			...
			withGroups(groups) {
				return {
					groups,
					interrupt: this.snapshot.settings.interrupt
				};
			}
			/**
		...
		*/
		function cancelTurnOf(ctx, sessionId) {
			return async () => {
				const face = ctx.sessions.scope(sessionId)?.get("conversation");
